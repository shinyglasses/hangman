...
class Gameplay_Elements:
    #it is important to initate multiple self.variables that work for the class to make it easier
    def __init__(self):
        ...
        #If a function is called every frame and it defines a variable inside it, that variable will
        #  reset to its default value each time the function runs. That's because local variables only exist while 
        # the function is running and get reinitialized each frame.
    def create_underlines(self, word):
        ...
        x_end = first_starting_x + line_length #would be (200 + 40, because starting = 200 and line length takes up 40 px)
        
        #creates an underline for each letter
        for i in range(len(word) + 1): #idk self explanatory
            if i == 0:
    ...
    def check_if_correct_letter(self, word, letter):
        positions = []     #preferably a list because uses for loop?????? huh..
        for i in range(len(word)): #basically if letter in word[letters] the position would append! (?) append?? why?? wdym append??
            if letter in word[i]:
             positions.append(i)
        return positions
